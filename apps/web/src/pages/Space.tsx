...
export default function Space() {
  ...
  const { isSessionExpanded } = useWorkspaceLayout();
        ...
        {authLoading ? (
          ...
          <span className="text-sm text-zinc-600 dark:text-zinc-400">
            <Button variant="link" asChild>
              <Link to="/logto" onClick={(e) => { e.preventDefault(); login(); }}>登录</Link>
            </Button>
            {' '}后使用
          </span>
        )}
  ...
  const sessionList = (
    <div className={isSessionExpanded ? 'w-64 shrink-0 overflow-auto border-r border-zinc-200 dark:border-zinc-700 flex flex-col min-h-0' : 'shrink-0 overflow-auto border-b border-zinc-200 dark:border-zinc-700'}
      style={isSessionExpanded ? undefined : { maxHeight: '220px' }}
    >
      {sessionsLoading ? (
        ...
        <ul className="py-1">
          {sessions.map((s) => (
            <li key={s.id}>
              <Link
                to={`/space/${encodeURIComponent(s.id)}`}
