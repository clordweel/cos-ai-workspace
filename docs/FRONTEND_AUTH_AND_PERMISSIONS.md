...
- **多认证源与通用化**：见 `docs/archive/research/`（UNIFIED_AUTH_DESIGN、AUTH_GENERIC_LOGTO_DESIGN）；前端只消费「是否已登录」与能力位，不按认证源写死分支。
...
| **需认证** | 必须已登录；未登录时引导去登录，不区分角色 | 发送消息、物料确认、需登录的应用扩展 |
...
- **点击/提交前**：在发送消息、物料确认等操作前，若逻辑上必须登录，先 `if (!useAuth().requireAuth()) return`，再发请求。
...
- 已在以下位置采用该模式：
  ...
  - `AppPanel.vue`：物料/密码等 401 → `requireAuth()` + 设置错误信息
