...
/** 联系人项：id 为 MXID，name 为展示名，avatar 可选 */
...
/**
 * 列出 Synapse 中未停用的用户，供联系人应用展示。
 * 使用 Admin API GET /_synapse/admin/v2/users，排除 deactivated，分页拉取至多 500 条。
 */
...
> {
  ...
  for (;;) {
    const url = `${config.matrix.baseUrl}${ADMIN_PATH}?from=${from}&limit=${limit}&deactivated=false`;
